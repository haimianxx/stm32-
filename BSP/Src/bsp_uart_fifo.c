...
static void UartIRQ(UART_T *_pUart)
{
	uint32_t isrflags   = READ_REG(_pUart->uart->SR);  // 读状态寄存器 RXNE、TXE、TC、ORE、FE 等状态标志位
	uint32_t cr1its     = READ_REG(_pUart->uart->CR1); // 读控制寄存器1 TXEIE、TCIE、RXNEIE 等中断使能位
	// uint32_t cr3its     = READ_REG(_pUart->uart->CR3); // 读控制寄存器3 CTSIE、EIE、DMAR、DMAT 等
	
	/* 处理接收中断  */
	if ((isrflags & USART_SR_RXNE) != RESET)
	{
		//注意：RXNE 不能软件清零，只能通过读 DR 自动清零。所以这个 if 进去后读一次 DR，出来后标志位就自动恢复了
		/* 从串口接收数据寄存器读取数据存放到接收FIFO */
		uint8_t ch;

		ch = READ_REG(_pUart->uart->DR); // 读数据寄存器 → 自动清 RXNE
		_pUart->pRxBuf[_pUart->usRxWrite] = ch; // 存入环形缓冲区
	...
	/* 处理发送缓冲区空中断 */
	if ( ((isrflags & USART_SR_TXE) != RESET)   // SR 寄存器: TXE 标志 = 1 (数据寄存器空)
		&& (cr1its & USART_CR1_TXEIE) != RESET) // CR1 寄存器: 用于决定是否允许TXE标志触发中断
	{
		...
		if (_pUart->usTxCount == 0) 
		{
			...
			CLEAR_BIT(_pUart->uart->CR1, USART_CR1_TXEIE);  // 关 TXE 中断(不允许 TXE 标志触发中断)

			/* 使能数据发送完毕中断 */
			//USART_ITConfig(_pUart->uart, USART_IT_TC, ENABLE);
			SET_BIT(_pUart->uart->CR1, USART_CR1_TCIE); // 开 TC 中断 移位寄存器所有位（含停止位）全部从引脚发出时
		}
		else
		{
			_pUart->Sending = 1;  // 标记"正在发送"
			
			/* 从发送FIFO取1个字节写入串口发送数据寄存器 */
			//USART_SendData(_pUart->uart, _pUart->pTxBuf[_pUart->usTxRead]);
			_pUart->uart->DR = _pUart->pTxBuf[_pUart->usTxRead]; //从 FIFO 取1字节写入 DR
			if (++_pUart->usTxRead >= _pUart->usTxBufSize)  // 读指针推进+回绕
			{
				_pUart->usTxRead = 0;
			}
			_pUart->usTxCount--;// 待发送计数-1
		}

	}
	/* 数据bit位全部发送完毕的中断 */
	if (((isrflags & USART_SR_TC) != RESET) 		// SR: TC 标志 = 1（发送真正完成）
		&& ((cr1its & USART_CR1_TCIE) != RESET))	// CR1: 用于决定是否允许TC 标志触发中断
	{
		//if (_pUart->usTxRead == _pUart->usTxWrite)
		if (_pUart->usTxCount == 0)
		{
...
*********************************************************************************************************
*	函 数 名: __io_putchar
*	功能说明: 重定义__io_putchar，printf底层会调用此函数输出字符
*	形    参: ch - 要输出的字符
*	返 回 值: 输出的字符
*********************************************************************************************************
*/
